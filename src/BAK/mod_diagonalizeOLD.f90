...
SUBROUTINE DIAGONALIZE( dimham, maxnonzero, hami, hamj, ham,      &
     &                  numevals, evals, numevecs, evecs )
  USE mod_indexx
  IMPLICIT NONE
! finds eigenvalues and eigenvectors of the Hamiltonian
...
INTEGER, INTENT(IN) :: maxnonzero
INTEGER, INTENT(IN) :: hami(dimham+1)
INTEGER, INTENT(IN) :: hamj(maxnonzero)
COMPLEX*16, INTENT(IN) :: ham(maxnonzero)
INTEGER, INTENT(IN) :: numevals
INTEGER, INTENT(IN) :: numevecs
REAL*8, INTENT(OUT) :: evals(numevals)
COMPLEX*16, INTENT(OUT) :: evecs(dimham,numevecs)
...
DO
  ...
  CALL avmult( n_ap, maxnonzero, hami, hamj, ham,                     &
