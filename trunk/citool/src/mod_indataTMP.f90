...
!  END TYPE ci_type_real
  ...
  INTEGER :: numspstates14, numspstates23
  INTEGER :: nc, nctmp
  INTEGER, PARAMETER :: SPINSHIFT= 12
  ...
  IF ( INDEX(fileinformat_coulomb,"dat") /= 0 ) THEN
    ...
    IF ( INDEX(fileinformat_coulomb,"real8") /= 0 ) THEN
      ...
      DO nc= 1, numci / 4
        nctmp= nc + (numci/4)
        ci(nctmp)%n1= ci(nc)%n1
        ci(nctmp)%n2= ci(nc)%n2 + SPINSHIFT
        ci(nctmp)%n3= ci(nc)%n3 + SPINSHIFT
        ci(nctmp)%n4= ci(nc)%n4
        ci(nctmp)%v=  ci(nc)%v
        nctmp= nc + 2*(numci/4)
        ci(nctmp)%n1= ci(nc)%n1 + SPINSHIFT
        ci(nctmp)%n2= ci(nc)%n2
        ci(nctmp)%n3= ci(nc)%n3
        ci(nctmp)%n4= ci(nc)%n4 + SPINSHIFT
        ci(nctmp)%v=  ci(nc)%v
        nctmp= nc + 3*(numci/4)
        ci(nctmp)%n1= ci(nc)%n1 + SPINSHIFT
        ci(nctmp)%n2= ci(nc)%n2 + SPINSHIFT
